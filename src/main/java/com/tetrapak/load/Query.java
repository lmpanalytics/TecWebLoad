...
//    'For most use cases it is recommended to use a single driver instance throughout an application.'
    // pw neo4j on the server is "s7asTaba". pw neo4j on my local machine is "Tokyo2000"
...
//            driver("bolt://" + HOSTNAME + "", AuthTokens.basic("neo4j", "neo4j"));
    ...
    public static void checkDatabaseIsRunning() {
        try (Session session = DRIVER.session()) {
            ...
            System.err.println(
                    "Exception in 'checkDatabaseIsRunning()':" + e + "\nProgram exits[1]...");
            System.exit(1);
        }
    }
...
//                Customer entities
                tx.run("CREATE CONSTRAINT ON "
                        + "(e:Entity) ASSERT e.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(cg:CustGrp) ASSERT cg.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(ct:CustType) ASSERT ct.name IS UNIQUE");

//                Markets
                tx.run("CREATE CONSTRAINT ON "
                        + "(C:Country) ASSERT C.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(m:Market) ASSERT m.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(mg:MarketGrp) ASSERT mg.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(cl:Cluster) ASSERT cl.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(gm:GlobalMarket) ASSERT gm.id IS UNIQUE");

//                Equipments
                tx.run("CREATE CONSTRAINT ON "
                        + "(eq:Equipment) ASSERT eq.id IS UNIQUE");
...
//              Parts
                tx.run("CREATE CONSTRAINT ON "
                        + "(p:Part) ASSERT p.id IS UNIQUE");
                tx.run("CREATE CONSTRAINT ON "
                        + "(pf:PartFamily) ASSERT pf.name IS UNIQUE");
            ...
            System.err.println(
                    "Exception in 'createIndexesAndConstraints()':" + e);
        }
    }
...
//              Parts
            ...
            System.err.println(
                    "Exception in 'dropIndexesAndConstraints()':" + e);
        }
    }
     ...
     *
     * @param INPUT_FILE_NAME name of csv file to be loaded
     */
...
//            System.out.printf("Path is %s%n ",path);
            String cypherPathFormat
                    = path.replace("\\", "/").replace("C:/", "file:///");
...
// Entities, Customer groups, Customer types and Countries
            String tx = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
...
// Markets, Market groups and Clusters
            String tx10 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    ...
                    + "MERGE (entity)-[:LINKED]->(market) "
                    + "MERGE (market)-[:IN]->(marketGrp) "
                    + "MERGE (marketGrp)-[:IN]->(cluster) "
                    + "MERGE (cluster)-[:IN]->(globalMarket)";

            session.run(tx10);

// Equipments
            String tx20 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    + "MATCH (entity: Entity {id: line.CustKey} )"
...
// Entities of Final customers
            String tx = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (e: Entity {id: line.CustKey} ) "
                    + "WITH e, line "
                    + "WHERE e.id IS NULL "
...
// Entities of Customer Groups
            String tx10 = "USING PERIODIC COMMIT "
                    ...
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (c: CustGrp {id: line.CustGroupKey} ) "
                    + "WITH c, line "
                    + "WHERE c.id IS NULL "
...
// Entities of Customer Types
            String tx20 = "USING PERIODIC COMMIT "
                    ...
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (c: CustType {name: line.CustTypeName} ) "
                    + "WITH c, line "
                    + "WHERE c.name IS NULL "
                    + "MERGE (:CustType {name: UPPER(line.CustTypeName)})";

            session.run(tx20);

// Customer Relationships Customer Key driven
            String tx30 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    + "MATCH (entity: Entity {id: line.CustKey} ) "
                    + "MATCH (custGrp: CustGrp {id: line.CustGroupKey} ) "
                    + "MATCH (custType: CustType {name: line.CustTypeName} ) "
                    + "MERGE (entity)-[:IN]->(custGrp) "
                    + "MERGE (entity)-[:IN]->(custType)";
...
//  Markets
            String tx40 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (m: Market {id: line.MarketKey} ) "
                    + "WITH m, line "
                    + "WHERE m.id IS NULL "
...
//  Market groups
            String tx50 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (m: MarketGrp {id: line.MarketGroupKey} ) "
                    + "WITH m, line "
                    + "WHERE m.id IS NULL "
...
//  Clusters
            String tx60 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    /* Add any missing nodes (the setdiff) between the db and the csv file */
                    + "OPTIONAL MATCH (c: Cluster {id: line.Cluster} ) "
                    + "WITH c, line "
                    + "WHERE c.id IS NULL "
...
//  Parts, Type and Quantity
            String tx80 = "USING PERIODIC COMMIT "
                    + "LOAD CSV WITH HEADERS FROM \"" + cypherPathFormat + "\" AS line FIELDTERMINATOR ';' "
                    + "WITH line "
                    + "MATCH (entity: Entity {id: line.CustKey} )  "
...
// Delete nodes where id or name is 'NA', except Country nodes
                tx100.run("MATCH (n) WHERE (n.id = 'NA' or n.name = 'NA') "
                        + "AND NOT (n:Country) "
                        + "DETACH DELETE n");
...
// Set Equipment properties that equals 0 to null
                tx100.run("MATCH (e:Equipment)"
                        + "WHERE e.heatNoPlatesTubes = 0 OR e.heatPercRegen = 0 "
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx = session.beginTransaction()) {
                ...
                while (result.hasNext()) {
                    ...
                    boolean hasTurnableDisk = r.get("hasTurnableDisk").
                            asBoolean();
...
//                    Set quantities for 3-Piston Machines
                ...
                Machines are classified as Aseptic if the model number ends with
                capital letter A, preceeded by either a space or a digit */
                    if (modelNumberEqmnt.matches("^\".*\\sA\"$") || modelNumberEqmnt.
                            matches("^\".*\\dA\"$")) {
                        pistonInterval = 1500;
                    ...
                    /* Increase part quantities and Crankcase family intervals for 5-Piston machines. 
                Machines are classified as 5-Piston machines if the model number
                contains '40' or '350' */
                    if (modelNumberEqmnt.contains("40")
                            || modelNumberEqmnt.contains("350")) {
...
//                        Increase intervals
                    ...
                    /* Increase lifetime for Valves if type is Turnable Disk i.e.,
                 all models 150 or 200 (refer to TB No. 4, 2007-06), or 
                if TecBase indicates that the valve is of this type */
                    if (hasTurnableDisk == true
                            || modelNumberEqmnt.contains("150")
                            || modelNumberEqmnt.contains("200")) {
                        valveInterval = 2 * valveInterval;
                        valve = "TD";
                    }
                    equipmentMap.put(key,
                            new Homogeniser(piston, pistonInterval, pistonQty,
                                    pistonSeal, pistonSealInterval,
                                    pistonSealQty, compressionRing,
                                    compressionRingInterval, compressionRingQty,
                                    forcer, forcerInterval, forcerQty,
                                    impactRingInterval, impactRingQty,
                                    seatInterval, forcerSeatQty, valve,
                                    valveInterval, valveQty,
                                    valveSealingInterval, valveSealingQty,
                                    valveSeatInterval, valveSeatQty,
                                    plainBearingInterval, plainBearingQty,
                                    rollerBearingInterval, rollerBearingQty,
                                    bellowsInterval, bellowQty, pistonSeal));

                }
            }
            System.out.printf("%s > Built %s Homogenisers\n", LocalDateTime.
                    now(), equipmentMap.size());
        } catch (ClientException e) {
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx = session.beginTransaction()) {
                StatementResult result = tx.run(
                        "MATCH (e:Equipment) WHERE e.machSystem = {term1} OR e.machSystem = {term2}\n"
                        + "RETURN e.id AS ID ",
                        Values.parameters("term1", "CENTRI", "term2",
                                "SEPARATOR"));
                tx.success();
                while (result.hasNext()) {
                    Record r = result.next();
                    ...
                    //                IM kit four changes per year or every 2000h, whatever comes first
                    int intermediateKitInterval = 2_000;
//                M kit one change per year or every 8000h, whatever comes first
                    int majorKitInterval = 8_000;
//                OWMC once a year
                    int oWMCInterval = 8_000;
                    int fFInterval = 24_000;

                    equipmentMap.put(key, new Separator(intermediateKitInterval,
                            majorKitInterval, oWMCInterval, fFInterval));
                }
            }
                    ...
                    if (PHE_FAMILY_MAP.containsKey(equipmentMtrlKey)) {
                        switch (PHE_FAMILY_MAP.get(equipmentMtrlKey).getModel()) {
                            case "C":
                                ...
                                equipmentMap.put(key,
                                        new PlateHeatExchanger("C",
                                                numberOfPlatesC, 12_000, 28_000,
                                                100));
                                break;
                            case "M":
                                if (numberOfPlates > 0) {
                                ...
                                equipmentMap.put(key,
                                        new PlateHeatExchanger("M",
                                                numberOfPlatesM, 12_000, 28_000,
                                                100));
                                break;
                            case "H":
                                if (numberOfPlates > 0) {
                                ...
                                equipmentMap.put(key,
                                        new PlateHeatExchanger("H",
                                                numberOfPlatesH, 12_000, 28_000,
                                                100));
                                break;
                            case "P":
                                if (numberOfPlates > 0) {
                                ...
                                equipmentMap.put(key,
                                        new PlateHeatExchanger("P",
                                                numberOfPlatesP, 12_000, 28_000,
                                                100));
                                break;
                            default:
                                break;
                        }
//                    If map doesn't contain a mapping then assign family 'N/A'
                    } else {
                        equipmentMap.put(key, new PlateHeatExchanger("N/A",
                                80, 12_000, 28_000, 100));
                    }
                }
            }
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx = session.beginTransaction()) {
                ...
                while (result.hasNext()) {
                    ...
                    int percentageRegeneration = r.get("percentageRegeneration").
                            asInt();
                    String equipmentMtrlKey = r.get("material").asString();
                ...
                mapping out of a total installed base of 6 500 THEs */
                    if (THE_FAMILY_MAP.containsKey(equipmentMtrlKey)) {
                        switch (THE_FAMILY_MAP.get(equipmentMtrlKey).getModel()) {
                            case "C":
                                equipmentMap.put(key, new TubularHeatExchanger(
                                        "C", nonRegTubes, regTubes, holdingTubes,
                                        6_000, 6_000));
                                break;
                            case "MT":
                                equipmentMap.put(key, new TubularHeatExchanger(
                                        "MT", nonRegTubes, regTubes,
                                        holdingTubes,
                                        6_000, 6_000));
                                break;
                            case "CIP":
                                equipmentMap.put(key, new TubularHeatExchanger(
                                        "CIP", 1, 0, 0, 6_000, 6_000));
                                break;
                            default:
                                break;
                        }
//                    If map doesn't contain a mapping then assign family 'N/A'
    ...
    private static Map<String, FrigusFreezer> buildFrigusFreezers() {
        ...
        FRIGUS_FREEZER_FAMILY_MAP.put("3094309-0100",
                new FrigusFreezerFamily("300"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3094310-0100",
                new FrigusFreezerFamily("600"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3094311-0100",
                new FrigusFreezerFamily("1200"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137675-0100",
                new FrigusFreezerFamily("500"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137676-0100",
                new FrigusFreezerFamily("1000"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137677-0100",
                new FrigusFreezerFamily("2000"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137678-0100",
                new FrigusFreezerFamily("3000"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137679-0100",
                new FrigusFreezerFamily("4000"));
        FRIGUS_FREEZER_FAMILY_MAP.put("3137715-0100",
                new FrigusFreezerFamily("700"));
...
//    Scraper                                
                                ...
                                equipmentMap.put(key, new FrigusFreezer(
                                        modelType, kit1000Interval, kit1000Qty,
                                        kit3000Interval, kit3000Qty,
                                        kit6000Interval, kit6000Qty,
                                        kit12000Interval, kit12000Qty,
                                        newPumpInterval, newPumpQty,
                                        exchangePumpInterval, exchangePumpQty,
                                        kitExtendedInterval, kitExtendedQty,
                                        starWheelInterval, starWheelQty,
                                        coverInterval, coverQty,
                                        impellerInterval, impellerQty,
                                        gasketKitInterval, gasketKitQty,
                                        bushingInterval, bushingQty,
                                        scraperKnifeInterval, scraperKnifeQty,
                                        sparePartsInterval, sparePartsQty,
                                        shaftSealInterval, shaftSealQty,
                                        cylinderKitInterval, cylinderKitQty,
                                        gasketsInterval, gasketQty,
                                        dasherKitInterval, dasherKitQty,
                                        singlePartsInterval, singlePartQty));
...
//    Scraper                                
                        ...
                        /* If map doesn't contain a mapping then search modelNumberEqmnt by Regex on 
                    descending numbers. 
                        This results in 90% correct mapping
                    of the Frigus Freezers in the total installed base
                    of 6000 Freezers */
                    } else if (modelNumberEqmnt.contains("CK") || modelNumberEqmnt.
                            contains("GM") || modelNumberEqmnt.contains("BLUE") || modelNumberEqmnt.
                            contains("MARK")) {
...
//    Scraper                                
                        ...
                        equipmentMap.put(key, new FrigusFreezer(
                                ...
                                dasherKitInterval, dasherKitQty,
                                singlePartsInterval, singlePartQty));
                    } else if (modelNumberEqmnt.contains("KF") || modelNumberEqmnt.
                            contains("FRIGUS") || modelNumberEqmnt.contains(
                            "FREEZ")) {
//                    Create "generic" instance of most frequent model (KF 1000)
                        ...
                        /* Do nothing (exclude equipments from the 
                    Frigus Freezer mapping) */
                    }
...
//            Add Equipment type to the Equipment node label
                    String tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                            + "MERGE (: Piston:Function {id: " + key + ", pistonMtrl: UPPER(" + toCypherVariableFormat(
                                    v.getPiston()) + "), pistonDiameter: '#', serviceInterval: toInt(" + v.
                            getPistonInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getPistonQty() + ")}]->(e)"
                            + "MERGE (: PistonSeal:Function {id: " + key + ", pistonSeal: UPPER(" + toCypherVariableFormat(
                                    v.getPistonSeal()) + "), serviceInterval: toInt(" + v.
                            getPistonSealInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getPistonSealQty() + ")}]->(e)"
                            + "MERGE (: CompressionRing:Function {id: " + key + ", compressionRing: UPPER(" + toCypherVariableFormat(
                                    v.getCompressionRing()) + "), serviceInterval: toInt(" + v.
                            getCompressionRingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getCompressionRingQty() + ")}]->(e)"
                            + "MERGE (: Forcer:Function {id: " + key + ", forcerMtrl: UPPER(" + toCypherVariableFormat(
                                    v.getForcer()) + "), serviceInterval: toInt(" + v.
                            getForcerInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getForcerQty() + ")}]->(e)"
                            + "MERGE (: ImpactRing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getImpactRingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getImpactRingQty() + ")}]->(e)"
                            + "MERGE (: Seat:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getSeatInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getForcerSeatQty() + ")}]->(e)"
                            + "MERGE (: Valve:Function {id: " + key + ", valve: UPPER(" + toCypherVariableFormat(
                                    v.getValve()) + "), serviceInterval: toInt(" + v.
                            getValveInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getValveQty() + ")}]->(e)"
                            + "MERGE (: ValveSealing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getValveSealingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getValveSealingQty() + ")}]->(e)"
                            + "MERGE (: ValveSeat:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getValveSeatInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getValveSeatQty() + ")}]->(e)"
                            + "MERGE (: PlainBearing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getPlainBearingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getPlainBearingQty() + ")}]->(e)"
                            + "MERGE (: RollerBearing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getRollerBearingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getRollerBearingQty() + ")}]->(e)"
                            + "MERGE (: Bellow:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getBellowsInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getBellowQty() + ")}]->(e)"
...
//            Add Equipment type to the Equipment node label
                    String tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                            + "MERGE (: IMkit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getIntermediateKitInterval() + ")})-[:IN]->(e)"
                            + "MERGE (: MSkit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getMajorKitInterval() + ")})-[:IN]->(e)"
                            + "MERGE (: OWMCkit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getoWMCkitInterval() + ")})-[:IN]->(e)"
                            + "MERGE (: FFkit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getfFkitInterval() + ")})-[:IN]->(e)"
                            + " SET e:Separator:Equipment";
    ...
    public static void addPHE() {
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx1 = session.beginTransaction()) {
                for (Map.Entry<String, PlateHeatExchanger> entry : eqMap.
                        entrySet()) {
                    String key = entry.getKey();
...
//            Add Equipment type to the Equipment node label
                    ...
                    switch (v.getModelType()) {
                        case "C":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: CGasket:Function {id: " + key + ", gasketQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getGasketInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: CPlate:Function {id: " + key + ", plateQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getPlateInterval() + ")})-[:IN]->(e)"
                                    + " SET e:PHE:Equipment";
                            break;
                        case "M":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: MGasket:Function {id: " + key + ", gasketQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getGasketInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: MPlate:Function {id: " + key + ", plateQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getPlateInterval() + ")})-[:IN]->(e)"
                                    + " SET e:PHE:Equipment";
                            break;
                        case "H":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: HGasket:Function {id: " + key + ", gasketQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getGasketInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: HPlate:Function {id: " + key + ", plateQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getPlateInterval() + ")})-[:IN]->(e)"
                                    + " SET e:PHE:Equipment";
                            break;
                        case "P":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: PGasket:Function {id: " + key + ", gasketQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getGasketInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: PPlate:Function {id: " + key + ", plateQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getPlateInterval() + ")})-[:IN]->(e)"
                                    + " SET e:PHE:Equipment";
                            break;
                        default:
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: Gasket:Function {id: " + key + ", gasketQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getGasketInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: Plate:Function {id: " + key + ", plateQty: toInt(" + v.
                                    getNumberOfPlates() + "), serviceInterval: toInt(" + v.
                                    getPlateInterval() + ")})-[:IN]->(e)"
                                    + " SET e:PHE:Equipment";
                            break;
    ...
    public static void addTHE() {
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx1 = session.beginTransaction()) {
                for (Map.Entry<String, TubularHeatExchanger> entry : eqMap.
                        entrySet()) {
                    String key = entry.getKey();
...
//            Add Equipment type to the Equipment node label
                    ...
                    switch (v.getModelType()) {
                        case "C":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: Ctube:Function {id: " + key + ", nonRegTubeQty: toInt(" + v.
                                    getNumberOfNonRegenerativeTubes() + "), regTubeQty: toInt(" + v.
                                    getNumberOfRegenerativeTubes() + "), holdingTubeQty: toInt(" + v.
                                    getNumberOfHoldingTubes() + ")})-[:IN]->(e)"
                                    + "MERGE (: CoRing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getoRingInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: Seal:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getProdSealInterval() + ")})-[:IN]->(e)"
                                    + " SET e:THE:Equipment";
                            break;
                        case "MT":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: MTtube:Function {id: " + key + ", nonRegTubeQty: toInt(" + v.
                                    getNumberOfNonRegenerativeTubes() + "), regTubeQty: toInt(" + v.
                                    getNumberOfRegenerativeTubes() + "), holdingTubeQty: toInt(" + v.
                                    getNumberOfHoldingTubes() + ")})-[:IN]->(e)"
                                    + "MERGE (: MToRing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getoRingInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: Seal:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getProdSealInterval() + ")})-[:IN]->(e)"
                                    + " SET e:THE:Equipment";
                        ...
                        case "CIP":
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: CIPtube:Function {id: " + key + ", nonRegTubeQty: toInt(" + v.
                                    getNumberOfNonRegenerativeTubes() + ")})-[:IN]->(e)"
                                    + "MERGE (: CIPoRing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getoRingInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: Seal:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getProdSealInterval() + ")})-[:IN]->(e)"
                                    + " SET e:THE:Equipment";
                        ...
                        default:
                            tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                                    + "MERGE (: Tube:Function {id: " + key + ", nonRegTubeQty: toInt(" + v.
                                    getNumberOfNonRegenerativeTubes() + "), regTubeQty: toInt(" + v.
                                    getNumberOfRegenerativeTubes() + "), holdingTubeQty: toInt(" + v.
                                    getNumberOfHoldingTubes() + ")})-[:IN]->(e)"
                                    + "MERGE (: ORing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getoRingInterval() + ")})-[:IN]->(e)"
                                    + "MERGE (: Seal:Function {id: " + key + ", serviceInterval: toInt(" + v.
                                    getProdSealInterval() + ")})-[:IN]->(e)"
                                    + " SET e:THE:Equipment";
    ...
    public static void addFrigusFreezer() {
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx1 = session.beginTransaction()) {
                for (Map.Entry<String, FrigusFreezer> entry : eqMap.
                        entrySet()) {
                    String key = entry.getKey();
...
//            Add Equipment type to the Equipment node label
                    String tx = "MATCH (e: Equipment) WHERE e.id = " + key + " "
                            //                            Service kits
                            + "MERGE (: Kit1000:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getKit1000Interval() + ")})-[:IN {qty:  toInt(" + v.
                            getKit1000Qty() + ")}]->(e)"
                            + "MERGE (: Kit3000:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getKit3000Interval() + ")})-[:IN {qty:  toInt(" + v.
                            getKit3000Qty() + ")}]->(e)"
                            + "MERGE (: Kit6000:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getKit6000Interval() + ")})-[:IN {qty:  toInt(" + v.
                            getKit6000Qty() + ")}]->(e)"
                            + "MERGE (: Kit12000:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getKit12000Interval() + ")})-[:IN {qty:  toInt(" + v.
                            getKit12000Qty() + ")}]->(e)"
                            //                            Pump
                            + "MERGE (: NewPump:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getNewPumpInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getNewPumpQty() + ")}]->(e)"
                            + "MERGE (: ExchangePump:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getExchangePumpInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getExchangePumpQty() + ")}]->(e)"
                            //                            Pump spare parts
                            + "MERGE (: ExtendedKit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getKitExtendedInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getKitExtendedQty() + ")}]->(e)"
                            + "MERGE (: StarWheel:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getStarWheelInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getStarWheelQty() + ")}]->(e)"
                            + "MERGE (: Cover:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getCoverInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getCoverQty() + ")}]->(e)"
                            + "MERGE (: Impeller:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getImpellerInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getImpellerQty() + ")}]->(e)"
                            + "MERGE (: GasketKit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getGasketKitInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getGasketKitQty() + ")}]->(e)"
                            + "MERGE (: Bushing:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getBushingInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getBushingQty() + ")}]->(e)"
                            //                            Scraper
                            + "MERGE (: ScraperKnife:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getScraperKnifeInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getScraperKnifeQty() + ")}]->(e)"
                            //                            Cooling
                            + "MERGE (: SpareParts:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getSparePartsInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getSparePartsQty() + ")}]->(e)"
                            //                            Shaft
                            + "MERGE (: ShaftSeal:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getShaftSealInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getShaftSealQty() + ")}]->(e)"
                            //                            Cylinder
                            + "MERGE (: CylinderKit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getCylinderKitInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getCylinderKitQty() + ")}]->(e)"
                            + "MERGE (: Gaskets:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getGasketsInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getGasketQty() + ")}]->(e)"
                            //                            Dasher
                            + "MERGE (: DasherKit:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getDasherKitInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getDasherKitQty() + ")}]->(e)"
                            + "MERGE (: SingleParts:Function {id: " + key + ", serviceInterval: toInt(" + v.
                            getSinglePartsInterval() + ")})-[:IN {qty:  toInt(" + v.
                            getSinglePartQty() + ")}]->(e)"
    ...
    public static void getDBcontent() {
        ...
        try (Session session = DRIVER.session()) {
            ...
            try (Transaction tx1 = session.beginTransaction()) {
                ...
                StatementResult r1 = tx1.run(
                        "MATCH (n) RETURN count(n) AS nodes;");
                StatementResult r2 = tx1.run(
                ...
                while (r2.hasNext()) {
                    ...
                    System.out.printf("%s > DB contains %s relationships\n",
                            LocalDateTime.now(), record.get("relationships"));
                }
            }
        } catch (ClientException e) {
    ...
    public static void dropDBdata() {
        try (Session session = DRIVER.session()) {
            System.out.printf(
                    "%s > Delete Nodes and Relationships from DB\n",
                    LocalDateTime.now());
